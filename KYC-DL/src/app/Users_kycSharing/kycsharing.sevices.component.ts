...
export class KycsharingService {
    ...
    public getsharingasset(){
        return this.http.get('http://localhost:3000/api/SharingAsset',this.options).map(res=> res.json())
    }
    public getKycseeker(id){
        return this.http.get('http://localhost:3000/api/KYC_Seeker/'+id+'',this.options).map(res => res.json()).catch(this.handleError);
    }
