...
export class KycsharedService {
    ...
    public getKycRequests(){
        return this.http.get('http://localhost:3000/api/Kyc_Seekers_Requests',this.options).map(res=> res.json())
    }
    public kycseekerinfo(data){
        return this.http.get('http://localhost:3000/api/KYC_Seeker/'+data+'',this.options).map(res => res.json()).catch(this.handleError);
    }
    public response(data){
        return this.http.post('http://localhost:3000/api/Permission_To_My_Kyc',data,this.options).map(res => res.json()).catch(this.handleError);
    }
